...
%     分别 min-max 归一化得 geomMeanN_h、geomMeanB_h、resTotal_h（其中 resTotal 为累积消耗，resTotal_h=1−minmax(res)，越大越省）；
%     **主排序标量** econGeomScore = (geomMeanN_h + geomMeanB_h + resTotal_h) / 3（三项算术平均，越大越优），
%     各假设内按 econGeomScore 降序，平局按 strat 升序；lexRank=1 为最优。
%     与**门槛**比较仍计算 passCountEcon（0–3），仅作诊断输出，不参与 lexRank。
...
%   调用：build_modular_qs_simulink_model("RunDualCommunityDeceptionSweep",true)
%   d1,d2,d3（Table 1）仍写入 rows 仅作**稳态诊断**，不参与 lexRank；判据与 DualStableEps1/2/3 见下。
%   强度参数：双菌扫描使用 dual_qs_community_rhs（恒化器 + Monod + Hill 诱导/抑制 + 杀伤 omega(B)），
%   与 Karkaria, Fedorec & Barnes, Nat Commun 12, 672 (2021) 式 (9)–(16) 同构（无量纲 S=S/S0 书写）。
...
%
% 用法：
%   build_modular_qs_simulink_model              % 默认不弹出 Simulink 编辑窗（OpenModel=false）
%   build_modular_qs_simulink_model("ModelName","qs_modular_mspg")
%   build_modular_qs_simulink_model("OpenModel",true,"RunSim",true)  % 需要看图或改模型时再打开
%   Scope_Pop 图例：按 produce 矩阵自动命名为「全合作株」「只欺瞒性状1」等（非 Mux_Pop(k)）
...
%   build_modular_qs_simulink_model("enableFoodLimit",false)  % 关闭「公共物=食物」限制（退化为原 dx）
%   build_modular_qs_simulink_model("foodAccessMin",1)  % 与旧版相同：不按产量区分取食权
%   build_modular_qs_simulink_model("foodAccessMin",0.08,"foodAmbient",0.015)  % 全瞒株更难蹭饭
%   build_modular_qs_simulink_model("RunDualCommunityDeceptionSweep",true,"OpenModel",false,"SaveModel",false)
%
...
opts = struct( ...
    ...
    "DualFigureExportDir", "figure", ...  % 相对脚本目录；也可传绝对路径
...
mwPop = get_param(modelName, "ModelWorkspace");
producePop = evalin(mwPop, "produce");
popLabels = local_strainEcologyLabels(producePop);
local_setMuxPopSignalNames(modelName, M, popLabels);
...
if opts.RunSim
    sim(modelName);
    local_plotPopTimeSeries(M, popLabels);
end
...
if isempty(opts.need_vec)
    ...
    if numel(n0) ~= M
        error("build_modular_qs_simulink_model:BadNeed", "need_vec 长度须等于 NumStrains (%d)。", M);
    end
end
...
sweepVals = sweepVals(:).';
mw = get_param(modelName, "ModelWorkspace");
popLabels = local_strainEcologyLabels(evalin(mw, "produce"));
n = numel(sweepVals);
stdN = nan(1, n);
meanFrac = nan(M, n);
for k = 1:n
...
cols = lines(M);
for j = 1:M
    plot(sweepVals, meanFrac(j, :), "-", "Color", cols(j,:), "LineWidth", 1.5, "DisplayName", popLabels{j});
end
hold off;
ylim([0 1]);
grid on;
...
function labels = local_strainEcologyLabels(produce)
%LOCAL_STRAINECOLOGYLABELS 由 produce 矩阵生成 Scope/图例用的生态角色短名
...
for j = 1:M
    ...
    elseif ~any(active)
        labels{j} = "纯搭便车";
    elseif sum(~active) == 1
        labels{j} = sprintf("只欺瞒性状%d", find(~active, 1));
    else
        cheatIdx = find(~active);
        labels{j} = sprintf("菌株%d（欺瞒性状%s）", j, strjoin(string(cheatIdx), ","));
    end
end
end

function local_setMuxPopSignalNames(modelName, M, labels)
%LOCAL_SETMUXPOPSIGNALNAMES 为 Mux_Pop 各输入线命名，Scope 图例显示生态角色
if numel(labels) < M
    error("build_modular_qs_simulink_model:BadLabels", "popLabels 长度须为 %d。", M);
end
muxPath = sprintf("%s/Mux_Pop", modelName);
ph = get_param(muxPath, "PortHandles");
for j = 1:M
    inLine = get_param(ph.Inport(j), "Line");
    if inLine > 0
        set_param(inLine, "Name", char(labels{j}));
    end
end
end

function local_plotPopTimeSeries(M, labels)
%LOCAL_PLOTPOPTIMESERIES RunSim 后按生态角色绘制丰度（与 Scope_Pop 一致）
if ~evalin("base", "exist('qs_sim_pop','var')")
    return;
end
[t, Y] = local_structPopToMatrix(evalin("base", "qs_sim_pop"), M);
...
figure("Name", "菌群丰度（生态角色）", "Color", "w");
hold on;
cols = lines(M);
for j = 1:M
    plot(t, Y(:, j), "LineWidth", 1.5, "Color", cols(j, :), "DisplayName", labels{j});
end
hold off;
grid on;
xlabel("时间");
...
% 组内：归一化 N_h、B_h、res_h 的算术平均 (N_h+B_h+res_h)/3
...
switch threshMode
    ...
    case 'fixed'
        ...
        actR = isfinite(thR0);
        if ~(actN || actB || actR)
            warning("build_modular_qs_simulink_model:DeceptionThresh", ...
    ...
    otherwise
        error("build_modular_qs_simulink_model:BadDeceptionEconThreshMode", ...
...
if ~any(ok)
    warning("build_modular_qs_simulink_model:DualSweepEmpty", "双菌扫描无有效解，跳过保存与作图。");
    return;
end
...
switch stratId
    ...
    case 4
        ...
        k2 = base;
    otherwise
        error("build_modular_qs_simulink_model:BadStrat", "stratId 须为 1..4。");
end
end
...
if nargin < 2 || ~iscell(argv) || numel(argv) < 8 %#ok<*ISCELL>
    error("build_modular_qs_simulink_model:HeatBarArgs", ...
