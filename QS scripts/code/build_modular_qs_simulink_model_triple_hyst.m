...
%
% 【三信号与调控拓扑】与 triple 构建一致：枚举 **18 种**调控拓扑（triple_qs_topology18.m，A1 行对 B1–B3 的 ±1/0 母库）。
...
%LOCAL_RUNTRIPLECOMMUNITYDECEPTIONSWEEP 18 种调控拓扑 × 产率策略（默认 8^3，可选 constrained138 / random512）；triple_qs_community_rhs / _hyst（11/14 维）
if ~isempty(scriptDir)
    addpath(scriptDir);
end
...
if echoCmd && (useConstrained138 || useRandom512)
    fprintf(1, '[triple sweep hyst] 产率扫描模式=%s | 每拓扑策略数=%d | 总 ODE=%d\n', char(prodSweepMode), nPerHyp, nRun);
end
wbH = [];
...
% rowsIn：可选；nPerHyp==138 且含 prodOrbit8 时额外导出横轴 T1–T8 的 *_orbit8T_heatmap
...
try
    for hi = 1:nHyp
        ...
        if nPerHyp == 138 && istable(rowsIn) && ismember('prodOrbit8', rowsIn.Properties.VariableNames) ...
