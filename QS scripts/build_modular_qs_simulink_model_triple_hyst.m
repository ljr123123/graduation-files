...
%
% 【三信号与调控】与 triple 一致：**固定** A1→(B1,B2,B3) 调控矩阵；扫描主轴为产率类 **P1–P8（=T1–T8）**，每类内列举该轨全部标号产率策略。
...
%   TripleSweepFastApprox,false（默认）：true 时即使未开启 OwnBPromotesNi，也启用「非迟滞+促进」同档的快速 ODE 近似（RelTol/AbsTol、Tend、Nt_ref、ResDot）；迟滞+促进仍走更激进的 HystProm 档。
...
%
% 【欺骗结构】默认 **constrained138**（8^3 全空间传 TripleProductionSweepMode='full512'）；码 i-j-k 每位 0..7（A1/A2/A3 bitmask）；大图扫描可仅导出热图（TripleLargeSweepHeatmapOnly）。
...
% geomMeanN/B 为三菌末段几何平均 (N1·N2·N3)^(1/3)、(B1·B2·B3)^(1/3)；passCountEcon 仅诊断，不参与 lexRank。
%
...
opts = struct( ...
    ...
    "TripleSweepFastApprox", false, ... % true：默认 B 抑制 N 时也启用快速近似（见文件头）
    ...
    "TripleLargeSweepHeatmapOnly", true, ... % 与 TripleSweepBarsP8Only 互斥见导出函数
    "TripleSweepBarsP8Only", true, ... % nHyp=8、前缀 P：仅 4 张 P1–P8 汇总柱图；每类列数 nPerHyp 多为 36
    ...
    "TripleRandomStratTrials", 0, ...
    "TripleRandomStratSeed", 0, ...
...
%LOCAL_RUNTRIPLECOMMUNITYDECEPTIONSWEEP 固定调控 × 产率 P1–P8 分组（constrained138 / random512）；triple_qs_community_rhs / _hyst（11/14 维）
if ~isempty(scriptDir)
    addpath(scriptDir);
end

topologyReductionStr = "orbit8_fixedReg";

legacyIv = logical(opts.TripleSweepLegacyIv);
hypFigPref = 'H';
orbitStratCell = cell(8, 1);
orbitMaxPer = 36;
if ~legacyIv
    MfullOr = double(triple_qs_production_orbit8('table'));
    for kOr = 1:8
        orbitStratCell{kOr} = MfullOr(MfullOr(:, 4) == kOr, 1:3);
    end
    orbitMaxPer = max(cellfun(@(c) size(c, 1), orbitStratCell));
end
...
if legacyIv
    ...
    nStrat = 8;
    hypFigPref = 'P';
    effFixed8 = local_tripleFixedRegulationEffDefault();
    nHyp = 8;
    effMats = cell(8, 1);
    oLab0 = triple_qs_production_orbit8('labels');
    hypLabels = cell(8, 1);
    for kOr = 1:8
        effMats{kOr} = effFixed8;
        hypLabels{kOr} = sprintf('P%d（T%d）%s', kOr, kOr, char(string(oLab0{kOr})));
    end
end
...
% --- 产信号子集扫描：constrained138（按 P1–P8 分组 + 占位）| random512 | legacy 下 4^3 ---
...
elseif useRandom512
    ...
    error("build_modular_qs_simulink_model_triple_hyst:TripleProductionSweepMode", ...
        "非 legacy 模式仅支持 TripleProductionSweepMode='constrained138' 或 'random512'（已移除 18×full512 扫描）。");
end
nRun = nHyp * nPerHyp;
...
useSweepFastOdeHystProm = ownBPromotes && fastOdeIfProm && useHystSweep;
relTolSweep = 1e-4;
absTolSweep = 1e-9;
...
elseif echoCmd && useSweepFastOde
    fprintf(1, '[triple sweep hyst] 快速档 ODE：%s RelTol=%.4g AbsTol=%.4g Tend=%.4g/%.4g Nt_ref=%d ResDot≤%d\n', ...
        sweepOdeKind, relTolSweep, absTolSweep, Tend, Tend0, Nt_ref, resDotMaxSamples);
end
if echoCmd && (useConstrained138 || useRandom512)
    fprintf(1, '[triple sweep hyst] 产率扫描模式=%s | 每组策略列数=%d | 总 ODE=%d（%s1–%s%d 分组）\n', ...
        char(prodSweepMode), nPerHyp, nRun, hypFigPref, hypFigPref, nHyp);
end
wbH = [];
...
for hi = 1:nHyp
    ...
    for ri = 1:nPerHyp
        ...
        if echoCmd && (idx == 1 || idx == nRun || mod(idx, wbStep) == 0)
            fprintf(1, '[triple sweep hyst] %d/%d (%.1f%%) %s%d 策略 (%d,%d,%d) 积分中…\n', ...
                idx, nRun, 100 * idx / nRun, hypFigPref, hi, a, b, c);
        end
        if showWb && ~isempty(wbH) && (idx == 1 || idx == nRun || mod(idx, wbStep) == 0)
            try %#ok<TRYNC>
                waitbar(idx / nRun, wbH, sprintf('三菌扫描 %d/%d（%.1f%%）| %s%d 策略 (%d,%d,%d)', ...
                    idx, nRun, 100 * idx / nRun, hypFigPref, hi, a, b, c));
                drawnow;
            catch %#ok<CTCH>
            end
...
tripleSweepMeta = struct( ...
    ...
    "evaluationRank", "per-prodRateClass P1..P8: econGeomScore=(N_h+B_h+res_h)/3 desc, strat asc; passCountEcon 仅诊断", ...
    "legacyIv", legacyIv, ...
    ...
    "nPerHyp", nPerHyp, ...
    "hypothesisFigurePrefix", hypFigPref, ...
    "fixedRegulationMatrix", effMats{1}, ...
    "prodRateSweepAxis", "P1_P8_orbit8" ...
    );
...
figPaths = local_exportTripleSweepFigures6(scriptDir, opts, hypLabels, nHyp, nPerHyp, ...
    t_ref, GnMat, GbMat, ResDotMat, tailGn, tailGb, tailResDot, stratPairLabels, n16, xLabBarStrat, nStrat, rows, hypFigPref);
...
if height(bestPaper) >= 1
    ...
    if hiX >= 1 && hiX <= numel(hypLabels)
        ...
        labX = sprintf('%s%d', hypFigPref, hiX);
    end
    scX = rX.stratCode;
    if iscell(scX)
        codeX = char(scX{1});
...
if ~legacyIv && numel(oLab) >= 8
    fprintf("\n=== 各产率类 P1–P8：固定调控下组内最优欺骗（econGeomScore）===\n");
    for oid = 1:8
        if ~isfinite(bestScore(oid, oid)) || isempty(bestCell{oid, oid})
            continue;
        end
        fprintf("P%d（T%d）| %s | score=%.4g | 最优码 %s\n", ...
            oid, oid, char(string(oLab{oid})), bestScore(oid, oid), bestCell{oid, oid});
    end
end
...
fprintf("\n=== 综合评价（组内归一三项的算术平均 econGeomScore）：各产率类 P1–P8 的 lexRank=1 ===\n");
fprintf("原始指标：geomMeanN、geomMeanB、resTotal；门槛模式「%s」（passCountEcon 仅诊断）\n", threshMode);
if strcmp(threshMode, 'median16')
    fprintf("门槛：各产率类组内中位数（N、B 为 ≥；累积资源为 ≤）→ passCountEcon=0..3\n");
...
for hi = 1:nHyp
    ...
    if strcmp(threshMode, 'none')
        fprintf("%s%d %s\n  -> 最优：N1=%s N2=%s N3=%s | 码 %s | lexRank=1/%d | econGeomScore=%.4g\n", ...
            hypFigPref, hi, hypLabels{hi}, s1, s2, s3, stratCodeDisp, height(subV), r.econGeomScore);
    else
    ...
    if strcmp(threshMode, 'median16') && hi <= numel(econThreshByHypothesis)
        ...
        fprintf("     本产率类门槛：geomN≥%.4g geomB≥%.4g res≤%.4g\n", eth.thGeomN, eth.thGeomB, eth.thResTotal);
    end
...
if isfield(gb, 'nCandidates') && gb.nCandidates >= 1
    ...
    fprintf("\n=== 全局「最最最优」（在 %d 个各产率类 lexRank=1 候选中，按 econGeomScore 等规则取第一）===\n", gb.nCandidates);
    fprintf("最优产率类：**P%d**（同 T%d）\n", gb.hypothesis, gb.hypothesis);
    fprintf("%s\n", gb.topologyLabel);
...
function M = local_tripleFixedRegulationEffDefault()
% 与原 triple_qs_topology18 最后一条「三群体：B2B3促进 B1抑制」相同的 A1→B 作用矩阵（A2/A3 行全零）
M = [-1, 1, 1; 0, 0, 0; 0, 0, 0];
end
...
function outPath = local_tripleExportOneBar64(figRoot, hi, tag, tailVec, stratPairLabels, xLabBarStrat, ylabBar, ttlLine, hypTitle, res, fmt, hypFigPref)
%LOCAL_TRIPLEEXPORTONEBAR64 64 柱：短数字码横轴 + 宽图 + 脚注码本
if nargin < 12 || isempty(hypFigPref)
    hypFigPref = 'H';
else
...
try %#ok<TRYNC>
    ...
    outPath = fullfile(figRoot, sprintf("%s%d_%s_bar.%s", hypFigPref, hi, tag, fmt));
...
function outPath = local_tripleExportCubeMetric(figRoot, hi, tag, tailVec64, hypTitle, res, fmt, ttlLong, hypFigPref)
%LOCAL_TRIPLEEXPORTCUBEMETRIC 2×2 子图：固定 N3 码，平面为 N1×N2（均为 0–3）
if nargin < 10 || isempty(hypFigPref)
    hypFigPref = 'H';
else
    hypFigPref = char(string(hypFigPref));
end
...
outPath = fullfile(figRoot, sprintf("%s%d_%s_N1N2slices.%s", hypFigPref, hi, tag, fmt));
try %#ok<TRYNC>
    local_dualTryExportgraphics(f, outPath, res);
catch ME %#ok<NASGU>
    close(f);
...
function figPaths = local_exportTripleSweepFigures6(scriptDir, opts, hypLabels, nHyp, nPerHyp, ...
    t_ref, GnMat, GbMat, ResDotMat, tailGn, tailGb, tailResDot, stratPairLabels, ylabIdxStr, xLabBarStrat, nStrat, rowsIn, hypFigPref)
%LOCAL_EXPORTTRIPLESWEEPFIGURES6 三菌：每产率类（P*）若干张图；nPerHyp>64 且 TripleLargeSweepHeatmapOnly 时仅 3 张热图
% rowsIn：可选；prodOrbit8 合法时可额外导出 T1–T8 聚合热图
% TripleSweepBarsP8Only 且 nHyp=8、hypFigPref=P：仅 4 张柱图（P1–P8）
if nargin < 16 || isempty(nStrat)
    nStrat = 4;
end
if nargin < 17
    rowsIn = [];
end
if nargin < 18 || isempty(hypFigPref)
    hypFigPref = 'H';
else
    hypFigPref = char(string(hypFigPref));
end
...
isP1toP8ProdSweep = (nHyp == 8) && strcmp(char(string(hypFigPref)), 'P');
if barsP8Only && isP1toP8ProdSweep && istable(rowsIn) && ismember('hypothesis', rowsIn.Properties.VariableNames) ...
        && height(rowsIn) >= nHyp
    try %#ok<TRYNC>
        filesOut = local_tripleExportP1toP8SummaryBars(figRoot, rowsIn, hypFigPref, res, fmt);
    catch ME
...
try
    for hi = 1:nHyp
        ...
        filesOut{end+1} = local_dualExportOneHeatmap(figRoot, hi, "geomN", t_ref, GnS, ylabIdxStr, ...
            "(N1·N2·N3)^{1/3}", "几何平均菌量 · 热图", hypTitle, res, fmt, hypFigPref); %#ok<AGROW>
        filesOut{end+1} = local_dualExportOneHeatmap(figRoot, hi, "geomB", t_ref, GbS, ylabIdxStr, ...
            "(B1·B2·B3)^{1/3}", "几何平均产物 · 热图", hypTitle, res, fmt, hypFigPref); %#ok<AGROW>
        filesOut{end+1} = local_dualExportOneHeatmap(figRoot, hi, "resRate", t_ref, RdS, ylabIdxStr, ...
            "dR/dt", "资源消耗率 · 热图", hypTitle, res, fmt, hypFigPref); %#ok<AGROW>
        if istable(rowsIn) && ismember('prodOrbit8', rowsIn.Properties.VariableNames) ...
            ...
            if numel(po) == nPerHyp && all(po >= 1 & po <= 8)
                ...
                filesOut{end+1} = local_tripleExportOrbit8Heatmap(figRoot, hi, "geomN", t_ref, Gn8, hypTitle, res, fmt, hypFigPref); %#ok<AGROW>
                filesOut{end+1} = local_tripleExportOrbit8Heatmap(figRoot, hi, "geomB", t_ref, Gb8, hypTitle, res, fmt, hypFigPref); %#ok<AGROW>
                filesOut{end+1} = local_tripleExportOrbit8Heatmap(figRoot, hi, "resRate", t_ref, Rd8, hypTitle, res, fmt, hypFigPref); %#ok<AGROW>
            end
        end
        if ~heatOnly
            filesOut{end+1} = local_tripleExportOneBar64(figRoot, hi, "geomN", tg, stratPairLabels, xLabBarStrat, ...
                "mean((N1·N2·N3)^{1/3}) 后30%", "几何平均菌量 · 末段均值", hypTitle, res, fmt, hypFigPref); %#ok<AGROW>
            filesOut{end+1} = local_tripleExportOneBar64(figRoot, hi, "geomB", tb, stratPairLabels, xLabBarStrat, ...
                "mean((B1·B2·B3)^{1/3}) 后30%", "几何平均产物 · 末段均值", hypTitle, res, fmt, hypFigPref); %#ok<AGROW>
            filesOut{end+1} = local_tripleExportOneBar64(figRoot, hi, "resRate", tr, stratPairLabels, xLabBarStrat, ...
                "mean(dR/dt) 后30%", "资源消耗率 · 末段均值", hypTitle, res, fmt, hypFigPref); %#ok<AGROW>
            if nStrat == 4
                filesOut{end+1} = local_tripleExportCubeMetric(figRoot, hi, "geomN", tg, hypTitle, res, fmt, ...
                    "几何平均菌量（末段）：4 子图 = N3 码 0–3，子图内横 N2 纵 N1", hypFigPref); %#ok<AGROW>
                filesOut{end+1} = local_tripleExportCubeMetric(figRoot, hi, "geomB", tb, hypTitle, res, fmt, ...
                    "几何平均产物（末段）：4 子图 = N3 码 0–3，子图内横 N2 纵 N1", hypFigPref); %#ok<AGROW>
                filesOut{end+1} = local_tripleExportCubeMetric(figRoot, hi, "resRate", tr, hypTitle, res, fmt, ...
                    "资源消耗率 mean(dR/dt)（末段）：4 子图 = N3 码 0–3，子图内横 N2 纵 N1", hypFigPref); %#ok<AGROW>
            end
        end
    end
catch ME
...
function outPath = local_tripleExportOrbit8Heatmap(figRoot, hi, tag, t_ref, Z8, hypTitle, res, fmt, hypFigPref)
%LOCAL_TRIPLEEXPORTORBIT8HEATMAP 横轴 T1–T8（产率轨），纵轴时间；同轨内多策略对时间序列取平均
if nargin < 9 || isempty(hypFigPref)
    hypFigPref = 'H';
else
    hypFigPref = char(string(hypFigPref));
end
...
try %#ok<TRYNC>
    ...
    outPath = fullfile(figRoot, sprintf('%s%d_%s_orbit8T_heatmap.%s', hypFigPref, hi, tg, fmt));
    local_dualTryExportgraphics(f, outPath, res);
catch ME
    close(f);
    rethrow(ME);
...
function outPath = local_dualExportOneHeatmap(figRoot, hi, tag, t_ref, Zmat, ylabIdxStr, cbLabel, ttlLine, hypTitle, res, fmt, hypFigPref)
...
try %#ok<TRYNC>
    ...
    outPath = fullfile(figRoot, sprintf("%s%d_%s_heatmap.%s", hypFigPref, hi, tag, fmt));
    local_dualTryExportgraphics(f, outPath, res);
catch ME %#ok<NASGU>
    close(f);
    rethrow(ME);
