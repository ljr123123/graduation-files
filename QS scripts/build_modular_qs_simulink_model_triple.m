...
%
% 【三信号 A1–A3、三群体 D1–D3（产物 B1–B3）】群落欺骗扫描：**不再枚举 18 种调控拓扑**；在 **固定** A1→(B1,B2,B3) 调控矩阵下，
%   按产率二部图 **P1–P8（=T1–T8，见 triple_qs_production_orbit8）** 分 8 组；每组内列举该轨全部标号产率策略（138 母集子集）并各跑一次 ODE。
%   默认 TripleProductionSweepMode='constrained138'：总列数 8×max(每组条数)，短组右侧以占位列填充（不参与排序）。
%   另支持 random512：8 组 × 每组随机若干 (stratN1,stratN2,stratN3)。**full512 已取消**（非 legacy 下会报错）。
...
%
% 【欺骗结构】默认 constrained138 按 **P1–P8 分组**；紧凑码 **i-j-k**（每位 stratN−1，0..7 为 A1/A2/A3 的 bitmask）；旧 64 组为 0..3。
%
...
opts = struct( ...
    ...
    "TripleLargeSweepHeatmapOnly", true, ... % nStrat^3>64 时仅导出热图（免 512 柱图）；与 TripleSweepBarsP8Only 互斥见导出函数
    "TripleSweepBarsP8Only", true, ... % true 且 nHyp=8、图前缀 P（产率类 P1–P8 扫描）：仅 4 张汇总柱图；nPerHyp 为每类列数（如 36），勿写死 138
...
%LOCAL_RUNTRIPLECOMMUNITYDECEPTIONSWEEP 固定调控 × 产率 P1–P8 分组（constrained138 / random512）；triple_qs_community_rhs（11 维）
if ~isempty(scriptDir)
    addpath(scriptDir);
end
...
elseif useRandom512
    ...
    error("build_modular_qs_simulink_model_triple:TripleProductionSweepMode", ...
...
if echoCmd && (useConstrained138 || useRandom512)
    fprintf(1, '[triple sweep] 产率扫描模式=%s | 每组策略列数=%d | 总 ODE=%d（%s1–%s%d 分组）\n', ...
...
for hi = 1:nHyp
    ...
    for ri = 1:nPerHyp
        ...
        if echoCmd && (idx == 1 || idx == nRun || mod(idx, wbStep) == 0)
            fprintf(1, '[triple sweep] %d/%d (%.1f%%) %s%d 策略 (%d,%d,%d) 积分中…\n', ...
...
% rowsIn：可选；当 nPerHyp 与 prodOrbit8 列合法时，可额外导出横轴 T1–T8 的聚合热图（*_orbit8T_heatmap）
% TripleSweepBarsP8Only 且 nHyp=8、hypFigPref=P：仅 4 张柱图（P1–P8），不导出任何热图/逐类柱图
